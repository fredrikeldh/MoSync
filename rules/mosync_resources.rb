...
class ConvertToPngTask < FileTask
	...
	def initialize(targetDir, resName, src, width, height)
		@width = width
		@height = height
		@src = src
		@resName = resName
...
class BMFontTask < FileTask
	def initialize(fontName, isBold, fontSize)
...
class MofTask < FileTask
	def initialize(targetDir, id, fontName, isBold, fontSize, color)
		...
		@prerequisites = [@bmTask, DirTask.new(targetDir)]
		@color = color
		@resName = "font_#{id}"
		super(name)
	end
...
class GeneratedLstTask < MemoryGeneratedFileTask
	...
	def initialize(resources, profile)
...
class BundleTask < FileTask
	def initialize(target, srcDir)
		@srcDir = srcDir
		@prerequisites ||= []
		([srcDir]+Dir["#{srcDir}/**/*"]).each do |file|
			#p file
			@prerequisites << FileTask.new(file)
		end
		super(target)
	end
	def cFlags
...
class RescompTask < FileTask
	include FlagsChanged
	def initialize(targetDir, src, platform)
		@targetDir = targetDir
		@src = src
		@platform = platform
		@prerequisites = [FileTask.new(src), DirTask.new(targetDir)]
		super(targetDir + '~tmpres.lst')
	end
	def cFlags
