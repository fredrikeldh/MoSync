...
#if 1
		int res = maAudioStop(mInstance[index]);
#else
		int res = maAudioPause(mInstance[index]);
#endif
		if(res != MA_AUDIO_ERR_OK && res != -100) {
			char buf[64];
			sprintf(buf, "maAudioStop %i\n", res);
			maPanic(res, buf);
		}
		// Rewinds the instance to the start of the sound
		res = maAudioSetPosition(mInstance[index], 0);
		lprintfln("maAudioSetPosition %i", res);
		if(res != MA_AUDIO_ERR_OK) {
			char buf[64];
			sprintf(buf, "maAudioSetPosition %i\n", res);
			maPanic(res, buf);
		}
		// Plays the sound
		res = maAudioPlay(mInstance[index]);
		lprintfln("maAudioPlay %i", res);
		if(res != MA_AUDIO_ERR_OK) {
			char buf[64];
			sprintf(buf, "maAudioPlay %i\n", res);
			maPanic(res, buf);
		}
	}
