...
static void outputMaapiCSharpWP8(const vector<Ix>& ixs, const Interface& maapi);
static void outputManagedCppCode(const vector<Ix>& ixs, const Interface& maapi);
static void outputPartialMachineFileCSharp(const vector<Ix>& ixs, const Interface& maapi);
...
static void outputMaapiCSharp(const vector<Ix>& ixs, const Interface& maapi) {
	...
	streamCSharpFunctionDelegates(maapiFile, maapi);
	maapiFile << "}\n\n";
	...
	for(size_t j=0; j<maapi.functions.size(); j++) {
		...
		for(size_t k=0; k<f.args.size(); k++) {
			...
			if(k != 0)
				maapiFile << ", ";
			outputCSharpSyscallArgTyped(maapiFile, i, a, maapi, "mCore.");
		}
		maapiFile << ")";
		...
		maapiFile << "\t\t\t\t\")";
		if(returnType != "noreturn" && returnType != "void")
			maapiFile << ": \"+mCore.GetRegisterValue(CoreInterpreted.Reg.r0)+\"";
		maapiFile << "\\n\");\n";
		maapiFile << "#endif\n";
		}
		// END LOG
	...
	for(size_t j=0; j<maapi.functions.size(); j++) {
		...
		for(size_t k=0; k<f.args.size(); k++) {
			...
			if(k != 0)
				maapiFile << ", ";
			if(argType == "double" || argType == "float") {
				maapiFile << "f" << fr;
				fr++;
			} else {
				outputCSharpSyscallNativeArg(maapiFile, i, "mCore.");
				i++;
			}
		}
	...
	for(size_t i=0; i<ioctls.size(); i++) {
		...
		for(size_t j=0; j<ioctl.functions.size(); j++) {
			...
			for(size_t k=0; k<f.args.size(); k++) {
				...
				if(k != 0)
					maapiFile << ", ";
				outputCSharpIoctlArgTyped(maapiFile, argindex, a, maapi, "mCore.");
			}
			maapiFile << ");\n";
...
void outputManagedCppCode(const vector<Ix>& ixs, const Interface& maapi)
{
...
void outputPartialMachineFileCSharp(const vector<Ix>& ixs, const Interface& maapi)
{
